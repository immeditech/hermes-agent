...
# ---------------------------------------------------------------------------
# immeditech fork patch: configurable callback bind address
# (oauth.redirect_bind — see docs/immeditech-fork.md)
# ---------------------------------------------------------------------------

class TestRedirectBind:
    """``oauth.redirect_bind`` selects the interface the listener binds to.

    Distinct from upstream's ``redirect_host`` (#63889), which only rewrites
    the hostname in the *advertised* redirect URI. We need the listener itself
    to be reachable, because our reverse proxy runs on a different host and
    cannot reach the agent LXC's loopback.
    """

    @pytest.fixture(autouse=True)
    def _restore_bind_host(self):
        import tools.mcp_oauth as mod

        original = mod._oauth_bind_host
        yield
        mod._oauth_bind_host = original

    def test_configure_callback_port_sets_bind_host_from_config(self):
        """redirect_bind is resolved into the module-level _oauth_bind_host."""
        import tools.mcp_oauth as mod

        mod._oauth_bind_host = "127.0.0.1"
        mod._configure_callback_port(
            {"redirect_port": 54322, "redirect_bind": "0.0.0.0"}
        )
    ...
    def test_empty_bind_host_falls_back_to_loopback(self):
    ...
    def test_find_free_port_accepts_explicit_host(self):
    ...
    def test_reserve_callback_port_honors_bind_host(self):
        """The TOCTOU reservation (#22161) lands on the configured interface.

        Regression guard for the 2026-08 upstream merge: the reservation was
        added upstream after our patch, so it is a second bind site that must
        follow redirect_bind — otherwise reservation and HTTPServer disagree.
        """
        import socket as sock
        import tools.mcp_oauth as mod
        ...
        port = mod._reserve_callback_port()
        try:
            reserved = mod._reserved_sockets[port]
            assert reserved.getsockname()[0] == "127.0.0.1"
        finally:
            mod._reserved_sockets.pop(port, None).close()

        # A bind host other than loopback is honoured too (0.0.0.0 is safe to
        # bind in CI; it is the address our HAProxy topology actually uses).
        mod._oauth_bind_host = "0.0.0.0"
        port = mod._reserve_callback_port()
        try:
            reserved = mod._reserved_sockets[port]
            assert reserved.getsockname()[0] == "0.0.0.0"
            # Sanity: the port really is bound on the wildcard interface.
            thief = sock.socket(sock.AF_INET, sock.SOCK_STREAM)
            with pytest.raises(OSError):
